...
extern unsigned char snesrom[];
extern int LoadFromDVD;
extern int LoadDVDFile( unsigned char *buffer );
...
#else
			/*** Nintendo Gamecube Memory ROM Loader ***/
			if ( LoadFromDVD == 0 ) {
				FileSize = snesromsize;
				memcpy(ptr, &snesrom[0], snesromsize);
			} else
				FileSize = LoadDVDFile( ptr );

			/*** Add DVD Load Routines here ***/
#endif		
...
//			Settings.MultiPlayer5Master = FALSE;
	...
	if (Settings.PAL)
	{
		timediffallowed = Settings.FrameTime = Settings.FrameTimePAL;
		Memory.ROMFramesPerSecond = 50;
	}
	else
	{
		timediffallowed = Settings.FrameTime = Settings.FrameTimeNTSC;
